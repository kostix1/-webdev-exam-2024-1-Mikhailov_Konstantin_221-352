...
const API_KEY_2GIS = '41dcbdc2-de60-497e-bd94-15fb9b528c89';
const PER_PAGE = 10;
...
function changeFieldRequestHandler(event) {
    ...
    if (excursionDate.value != '' && startTime.value != '') {
        ...
        if (option1.checked) {
            ...
            calculateTotalCost *= 1.30;
            option1amount.value = "+" + parseInt(calculateTotalCost - temp) + " рублей";
        } else {
            option1amount.value = ""
        };
        if (option2.checked) {
            let temp = calculateTotalCost;
            calculateTotalCost += 500 * numberOfPeople.value;
...
async function buttonChooseGuideHandler(event) {
    ...
    option2Desc.innerHTML = 'Стоимость увеличивается на 500 рублей для каждого посетителя';
